...
export class TeamWiseInteractionsDataSource implements DataSource<Inventory> {
  ...
  loadData(inventoryResource: InventoryResourceParameter) {
    ...
    this.sub$ = this.inventoryService.getTeamWiseInteractionsList(inventoryResource)
      .pipe(
        catchError(() => of([])),
