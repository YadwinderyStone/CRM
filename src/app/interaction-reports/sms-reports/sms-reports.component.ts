...
export class SmsReportsComponent extends BaseComponent implements OnInit {  
    toDate: any = new Date();
    ...
    displayedColumns: string[] = ['sMSRecipientName','smsSenderName','sMSRecipientNumber','created','sMSSentTime','sMSStatus','smsUniqueMsgId','sMSText','apiRes']
    ...
    constructor(
      ...
      this.inventoryResource.smsStatus = this.smsStatus
  
    }
  
    ngOnInit(): void {
      this.dataSource = new InteractionDataSource(this.interactionReportsService);
      this.dataSource.loadSMSData(this.inventoryResource);
    ...
    downloadExcel() {
      let url = `Excel/GetExcelFileForSmsQueueReport`
      this.isLoading = true;
      this.setParams();
