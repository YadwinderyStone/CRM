...
@Component({
  selector: 'app-email-resolution-reports',
  templateUrl: './email-resolution-reports.component.html',
  styleUrls: ['./email-resolution-reports.component.scss']
})
export class EmailResolutionReportsComponent extends BaseComponent implements OnInit {
  ...
  displayedColumns: string[] = ['interactionid', 'status','source','createdat','mailFromEmailId',
    'mailTo','mailSendDate','emailResolutionCommnet','resolutionCommnet'];
  ...
  downloadExcel() {
    ...
    this.interactionReportsService.get187InteractionsReportsExcelDowanload(url, this.inventoryResource).subscribe((res: any) => {
      ...
      a.href = url;
      a.download = 'EmailResolutionReports.xlsx';
